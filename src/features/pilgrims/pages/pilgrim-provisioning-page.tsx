import { AlertCircle, Copy, Download, Plus, QrCode, RefreshCw, Search, Share2, Trash2, Upload } from 'lucide-react'
...
export function PilgrimProvisioningPage() {
  ...
  const onProvisionOne = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await provisionPilgrim(selectedGroupId, form)
      setForm({
        full_name: '',
        phone_number: '',
        language: 'en',
  ...
  const pendingItems = useMemo(
    () => provisioningItems.filter((item) => item.status !== 'activated'),
    [provisioningItems],
  )
  ...
  const recentItems = useMemo(() => provisioningItems.slice(0, 24), [provisioningItems])
  ...
  return (
    <div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-12">
        <Card className="lg:col-span-7 lg:h-[450px] flex flex-col overflow-hidden">
          <CardHeader>
          ...
          <CardContent className="min-h-0 flex-1 space-y-6 overflow-y-auto">
            ...
            <form onSubmit={onProvisionOne} className="grid grid-cols-1 gap-3 md:grid-cols-4">
              ...
              <div className="md:col-span-4 flex flex-wrap items-center gap-2">
                ...
                <Button type="button" variant="outline" onClick={() => void refreshProvisioningStatus()} disabled={!selectedGroupId || isRefreshingStatus}>
                  <RefreshCw className={`mr-2 size-4 ${isRefreshingStatus ? 'animate-spin' : ''}`} />
                  Refresh status
                </Button>
              </div>
            </form>
        ...
        <Card className="lg:col-span-5 flex h-[450px] flex-col overflow-hidden">
          <CardHeader>
            <CardTitle>Pending Login Credentials</CardTitle>
            <CardDescription>Search, tap a pilgrim, and open the full code or QR when needed.</CardDescription>
          </CardHeader>
          <CardContent className="grid min-h-0 flex-1 grid-rows-[auto_1fr] gap-3 overflow-hidden">
            <div>
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="Search name, phone, code, status"
                  className="pl-9"
                />
              </div>
            </div>
            ...
            {filteredPendingItems.length === 0 ? (
              <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
                {searchQuery.trim()
                  ? 'No matching pending pilgrims found.'
                  : 'No pending credentials. All provisioned pilgrims in this group are activated.'}
              </div>
            ) : (
                  ...
                  <div
                    ...
                    onKeyDown={(event) => {
                      if (event.key === 'Enter' || event.key === ' ') {
                        event.preventDefault()
                        setSelectedProvisioningItem(item)
                      }
                    }}
                    className="cursor-pointer rounded-lg border border-border/70 bg-background p-3 transition-colors hover:border-accent/40 hover:bg-muted/30 focus:border-accent/40 focus:outline-none"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <p className="font-medium">{item.pilgrim.full_name}</p>
                        <p className="text-xs text-muted-foreground">
                          {item.pilgrim.phone_number} · {item.one_time_login.token ? 'code ready' : 'needs reissue'}
                        </p>
                      </div>
                      <span className={`rounded-md border px-2 py-1 text-xs font-medium capitalize ${getStatusClasses(item.status)}`}>
                        {item.status}
                      </span>
                    </div>
                    <div className="mt-2 grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">
                      <div className="rounded-md bg-muted/40 px-2 py-1">Issued: {formatDate(item.one_time_login.issued_at)}</div>
                      <div className="rounded-md bg-muted/40 px-2 py-1">Expires: {formatDate(item.one_time_login.expires_at)}</div>
                    </div>
                    <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                      <QrCode className="size-4" />
                      Click to open code, QR, and sharing tools
                    </div>
                    <div className="mt-2 flex items-center justify-end gap-2">
      ...
      <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>Track who has used their one-time code and who is still pending.</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <table className="w-full min-w-[720px] text-sm">
                  ...
                  <tbody>
                    {recentItems.map((item) => (
                      <tr key={item.pilgrim._id} className="border-b last:border-b-0">
                        <td className="px-3 py-2 font-medium">{item.pilgrim.full_name}</td>
                        <td className="px-3 py-2 text-muted-foreground">{item.pilgrim.phone_number}</td>
                        <td className="px-3 py-2">
                          <span className={`rounded-md border px-2 py-1 text-xs font-medium capitalize ${getStatusClasses(item.status)}`}>
                            {item.status}
                          </span>
                        </td>
                        <td className="px-3 py-2 text-muted-foreground">{formatDate(item.one_time_login.issued_at)}</td>
                        <td className="px-3 py-2 text-muted-foreground">{formatDate(item.one_time_login.used_at)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
